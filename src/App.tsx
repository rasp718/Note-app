...
function App() {
  ...
  const canvasRef = useRef<HTMLCanvasElement>(null); // For Matrix Rain
  ...
  const cycleFilter = () => {
      ...
      if (nextFilter !== 'all' && nextFilter !== 'secret') {
  ...
  // Minimalist Grayscale Pill
  const getHeaderPillDetails = () => {
    if (activeFilter === 'secret') {
        return { label: 'Hacker Mode', icon: <Terminal size={14} /> };
    }
    if (activeFilter === 'all') {
        return { label: 'All', icon: <LayoutGrid size={14} /> };
    }
    const cat = categories.find(c => c.id === activeFilter);
    return { 
        label: cat?.label || 'Unknown', 
        icon: <span className="text-sm leading-none">{cat?.emoji}</span>,
    };
  };
  ...
  return (
    <div className={`min-h-screen w-full bg-black text-zinc-100 font-sans ${currentTheme.selection} overflow-x-hidden ${currentTheme.font}`}>
        ...
        <header className="max-w-2xl mx-auto flex items-center justify-between px-4 py-3 pointer-events-auto relative">
            ...
            <div className={`transition-all duration-300 flex-shrink-0 ${isSearchExpanded ? 'w-0 opacity-0 overflow-hidden' : 'w-auto opacity-100'}`}>
                <button 
                    ...
                    className="w-10 h-10 bg-transparent flex items-center justify-center rounded-xl active:scale-95 transition-transform relative overflow-visible"
                >
                    {isStartup && (
                        <>
            ...
            {/* 2. Right Group: Settings, Search, Category */}
            <div className="flex items-center gap-1">
                
                {/* Settings */}
                 <button 
                    onClick={() => { setShowSettings(true); }} 
                    className={`w-10 h-10 flex items-center justify-center text-zinc-500 hover:text-orange-500 transition-all active:scale-95 ${isSearchExpanded ? 'w-0 opacity-0 overflow-hidden' : 'w-10 opacity-100'}`}
                >
                    <Settings size={20} />
                </button>
                ...
                <div className="relative flex items-center justify-end h-10">
                    ...
                    <button 
                        ...
                        className={`w-10 h-10 flex items-center justify-center text-zinc-500 hover:text-orange-500 transition-all active:scale-95 absolute right-0 ${isSearchExpanded ? 'opacity-0 pointer-events-none scale-50' : 'opacity-100 scale-100'}`}
                    >
                        <Search size={20} />
                    </button>
                     ...
                     <div 
                        className={`bg-zinc-900 border border-zinc-800 rounded-full flex items-center px-3 h-10 transition-all duration-300 origin-right ${isSearchExpanded ? 'w-[200px] opacity-100 shadow-lg' : 'w-10 opacity-0 pointer-events-none'}`}
                    >
                        ...
                        <input 
                            ...
                            className="bg-transparent border-none outline-none text-white text-sm w-full h-full placeholder:text-zinc-600 min-w-0"
                        />
                        <button onClick={() => { setSearchQuery(''); setIsSearchExpanded(false); }} className="p-1 text-zinc-500 hover:text-white flex-shrink-0">
                            <X size={14} />
                        </button>
                    </div>
                </div>
                ...
                <button 
                    onClick={cycleFilter}
                    className={`flex items-center gap-2 pl-3 pr-1 h-10 rounded-full text-zinc-500 hover:text-orange-500 transition-colors active:scale-95 select-none ${isSearchExpanded ? 'opacity-0 w-0 overflow-hidden' : 'opacity-100 w-auto'}`}
                >
      ...
      {/* --- MATRIX EFFECT LAYER (Hacker Mode) --- */}
      {showSecretAnim && (
        <canvas 
            ref={canvasRef}
      ...
      <div className={`pt-20 pb-0 px-4 max-w-2xl mx-auto flex flex-col gap-3 ${getAlignmentClass()}`}>
          ...
          {filteredNotes.map(note => (
              <div 
                  ...
                  className={`select-none touch-manipulation transition-all duration-300 active:scale-[0.99] ${editingNote?.id === note.id ? 'opacity-50 blur-[1px]' : 'opacity-100'}`}
              >
                  <NoteCard 
                      note={note} 
          ...
          {filteredNotes.length === 0 && (
              <div className="text-center py-20 border border-dashed border-zinc-900 rounded-lg col-span-full opacity-50 w-full">
                  ...
                  <p className="text-zinc-700 text-xs font-mono uppercase">
                      {activeFilter === 'secret' ? 'System Clean' : 'Start Typing...'}
                  </p>
              </div>
          )}
      ...
      <div className={`fixed bottom-0 left-0 right-0 z-50 p-3 pb-6 md:pb-3 pointer-events-none translate-y-0`}>
          <div className="max-w-2xl mx-auto pointer-events-auto flex flex-col gap-2">
            ...
            {/* Context Bar for Editing (Slides up) */}
            {editingNote && (
                <div className="flex items-center justify-between bg-zinc-900/90 backdrop-blur-xl border-t border-x border-zinc-800 rounded-t-2xl px-4 py-2 mb-[-10px] mx-1 animate-in slide-in-from-bottom-5 fade-in duration-200">
                    <div className="flex items-center gap-3 overflow-hidden">
                        <div className={`p-1.5 rounded-full bg-opacity-20 ${editingNote.category === 'secret' ? 'bg-red-500 text-red-500' : 'bg-blue-500 text-blue-500'}`}>
                            <PenLine size={12} />
                        </div>
                        <div className="flex flex-col">
                            <span className={`text-[10px] font-bold uppercase tracking-wider ${editingNote.category === 'secret' ? 'text-red-500' : 'text-blue-500'}`}>Editing Message</span>
                            <span className="text-xs text-zinc-400 truncate max-w-[200px]">{editingNote.text}</span>
                        </div>
                    </div>
                    <button onClick={handleCancelEdit} className="p-1 hover:bg-zinc-800 rounded-full text-zinc-500 hover:text-white transition-colors">
                        <X size={16} />
                    </button>
                </div>
            )}
            ...
            <div className="flex items-end gap-2 bg-black/50 backdrop-blur-sm rounded-3xl p-1">
                ...
                <div className="flex-1 bg-zinc-900 border border-zinc-800 rounded-2xl flex items-center px-4 py-2 focus-within:border-zinc-600 transition-colors gap-3 shadow-lg shadow-black/50 relative">
                    {imageUrl && (
                        <div className="relative flex-shrink-0 group/image">
                            ...
                            <button onClick={() => { setImageUrl(''); if(fileInputRef.current) fileInputRef.current.value = ''; }} className="absolute -top-1.5 -right-1.5 w-4 h-4 bg-red-500 text-white rounded-full flex items-center justify-center opacity-0 group-hover/image:opacity-100 transition-opacity shadow-sm">
                                <X size={10} />
                            </button>
                        </div>
                    )}
                    ...
                    <textarea 
                        ...
                        placeholder={editingNote ? "Edit message..." : (activeFilter === 'secret' ? "Inject code..." : t.typePlaceholder)} 
                        rows={1} 
                        onFocus={() => { setIsNoteFocused(true); setShowBars(true); scrollToBottom(); }}
                        ...
                        className={`w-full bg-transparent border-none text-white placeholder:text-zinc-600 focus:outline-none text-base md:text-sm resize-none max-h-32 py-0.5 ${
                            activeFilter === 'secret' ? 'font-mono text-green-500 placeholder:text-green-800' : ''
                        }`} 
                    />
                ...
                <button 
                    ...
                    className={`flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 active:scale-95 shadow-lg shadow-black/50 ${transcript.trim() || imageUrl ? `${currentTheme.bg} ${currentTheme.shadow} text-white` : 'bg-zinc-900 text-zinc-600 border border-zinc-800'}`}
                >
