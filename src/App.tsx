    ...
    <div className="min-h-screen w-full bg-black text-zinc-100 font-sans selection:bg-orange-500/30">
      <div className="fixed top-0 left-0 right-0 z-40 bg-black/95 backdrop-blur-xl border-b border-zinc-900 pb-2 shadow-lg">
        <header className="max-w-2xl mx-auto flex items-center gap-3 p-4 pb-2">
           ...
           <div className="flex-1 relative group"><Search className="absolute left-3 top-2.5 text-zinc-600 group-focus-within:text-white transition-colors" size={16} /><input type="text" placeholder={t.search} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-zinc-900 border border-zinc-800 rounded-full py-2.5 pl-9 pr-4 text-white focus:outline-none focus:border-white transition-all placeholder:text-zinc-600 placeholder:font-bold placeholder:uppercase placeholder:tracking-wider text-base" style={{ fontSize: '16px' }} /></div>
           <button onClick={() => { setShowSettings(true); setSettingsView('main'); }} className="flex-shrink-0 w-10 h-10 flex items-center justify-center rounded-full bg-zinc-900 border border-zinc-800 hover:border-zinc-700 text-zinc-500 hover:text-white transition-all active:scale-95"><Settings size={16} /></button>
        </header>
      ...
      <div className={`pt-36 pb-40 px-4 max-w-2xl mx-auto flex flex-col gap-3 ${getAlignmentClass()}`}>
          ...
          {filteredNotes.length === 0 && <div className="text-center py-20 border border-dashed border-zinc-900 rounded-lg col-span-full opacity-50 w-full"><LayoutGrid className="mx-auto text-zinc-800 mb-2" size={32} /><p className="text-zinc-600 text-xs font-bold uppercase tracking-wider">Database Empty</p></div>}
      </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 bg-black/95 backdrop-blur-xl border-t border-zinc-900 p-3 pb-6 md:pb-3 transition-all">
          <div className="max-w-2xl mx-auto flex items-end gap-2">
              ...
              <div className="flex-1 bg-zinc-900 border border-zinc-800 rounded-2xl flex items-center px-4 py-2 focus-within:border-zinc-600 transition-colors gap-3">
                  ...
                  <textarea ref={textareaRef} value={transcript} onChange={(e) => setTranscript(e.target.value)} onPaste={(e) => handlePaste(e)} placeholder={t.typePlaceholder} rows={1} className="w-full bg-transparent border-none text-white placeholder:text-zinc-600 placeholder:font-bold placeholder:uppercase placeholder:tracking-wider focus:outline-none text-base resize-none max-h-32 py-0.5" style={{ fontSize: '16px' }} />
              </div>
              <button onClick={saveNote} disabled={!transcript.trim() && !imageUrl} className={`flex-shrink-0 w-10 h-10 mb-0.5 rounded-full flex items-center justify-center transition-all duration-300 active:scale-95 ${transcript.trim() || imageUrl ? 'bg-orange-600 text-white shadow-[0_0_15px_rgba(234,88,12,0.5)]' : 'bg-zinc-900 text-zinc-600 border border-zinc-800'}`}>{transcript.trim() || imageUrl ? <ArrowUp size={18} strokeWidth={2.5} /> : <Plus size={18} strokeWidth={2.5} />}</button>
          </div>
      </div>
      ...
      {showSettings && (
         <div className="fixed inset-0 z-50 flex justify-center sm:items-center bg-black sm:bg-black/80 animate-in fade-in duration-200">
             <div className="w-full h-full sm:h-auto sm:max-w-md bg-black sm:border border-zinc-800 sm:rounded-2xl p-4 pt-12 sm:pt-6 shadow-2xl relative flex flex-col">
                <div className="flex justify-between items-center mb-6"><h2 className="text-[10px] font-bold uppercase tracking-widest text-zinc-600 flex items-center gap-2">{t.config}</h2><button onClick={() => setShowSettings(false)} className="w-8 h-8 flex items-center justify-center rounded-full bg-zinc-900 border border-zinc-800 text-zinc-500 hover:text-white hover:border-zinc-700 transition-all active:scale-95"><X size={14} /></button></div>
                {settingsView === 'main' ? (
                    <div className="flex flex-col gap-4">
                        <div className="flex-1 overflow-y-auto space-y-2">{categories.map((cat) => (<button key={cat.id} onClick={() => { setEditingCatId(cat.id); setSettingsView('icons'); }} className="w-full flex items-center gap-3 p-2.5 rounded-lg bg-zinc-900 border border-zinc-800"><div className="w-8 h-8 flex items-center justify-center rounded-full bg-black border border-zinc-800 text-sm grayscale">{cat.emoji}</div><p className="text-[10px] font-bold uppercase tracking-wider text-zinc-300">{getCategoryLabel(cat)}</p></button>))}</div>
                        <div className="pt-4 border-t border-zinc-900 space-y-2"><button onClick={() => { const data = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(notes)); const a = document.createElement('a'); a.href = data; a.download = 'backup.json'; a.click(); }} className="w-full py-2.5 bg-zinc-900 border border-zinc-800 text-zinc-400 text-[10px] font-bold uppercase tracking-widest flex items-center justify-center gap-2 hover:text-orange-500 hover:border-zinc-700 transition-colors"><Download size={14} /> {t.backup}</button><button onClick={() => signOut(auth)} className="w-full py-2.5 bg-zinc-900 border border-zinc-800 text-zinc-400 text-[10px] font-bold uppercase tracking-widest flex items-center justify-center gap-2 hover:text-orange-500 hover:border-zinc-700 transition-colors"><LogOut size={14} /> {t.logout}</button></div>
                    </div>
                ) : (
